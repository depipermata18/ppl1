...
// 🔒 Debug (opsional)
$show_debug = (isset($_GET['debug']) && $_GET['debug'] === 'rahasia123');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['otp'])) {
    ...
    if (!$user) {
        ...
        if ($user['otp_code'] !== $otp) {
            ...
            if ($show_debug) {
                $error .= "<br><br><strong style='color:#4F80FF;'>[DEBUG INFO - ZONA WAKTU: ASIA/JAKARTA]</strong><br>";
                $error .= "<small>";
                $error .= "🕒 Waktu sekarang: <strong>" . $now->format('Y-m-d H:i:s') . "</strong><br>";
                $error .= "⏳ Waktu expired: <strong>" . $expired->format('Y-m-d H:i:s') . "</strong><br>";
                $error .= "📊 Status: <span style='color:red;'>KADALUARSA</span>";
                $error .= "</small>";
            }
        } else {
            // ✅ OTP valid → tandai sesi dan hapus OTP
...
// ========== TAHAP 2: Update Password (setelah OTP valid) ==========
...
// Cek apakah sudah verifikasi OTP (dari sesi)
if (isset($_SESSION['otp_verified']) && $_SESSION['otp_verified'] === true && $_SESSION['reset_email'] === $email) {
    $otp_verified = true;
}
?>
